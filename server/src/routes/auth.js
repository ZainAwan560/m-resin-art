...
r.post('/login',limiter,async(req,res,next)=>{try{const {email,password}=req.body;const a=await Admin.findOne({email:email?.toLowerCase()});if(!a||!(await bcrypt.compare(password||'',a.passwordHash)))return res.status(401).json({message:'Invalid credentials'});a.lastLoginAt=new Date();await a.save();const token=jwt.sign({sub:a._id.toString(),role:a.role,email:a.email},process.env.JWT_SECRET,{expiresIn:'8h'});res.cookie('admin_token',token,{httpOnly:true,sameSite:'lax',secure:process.env.COOKIE_SECURE==='true',domain:process.env.COOKIE_DOMAIN||undefined,maxAge:8*60*60*1000});res.json({admin:{email:a.email,role:a.role}});}catch(e){next(e)}});
r.post('/logout',(req,res)=>{res.clearCookie('admin_token');res.json({ok:true})});
